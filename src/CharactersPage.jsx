...
a{color:inherit}
.chub{min-height:100vh;background:radial-gradient(circle at 20% 6%,var(--amber-glow),transparent 28%),linear-gradient(180deg,var(--bg-top),var(--bg-bottom) 72%);color:var(--paper);font-family:'Newsreader',Georgia,serif}
...
.chub-card{display:flex;flex-direction:column;gap:12px;padding:28px 24px 24px;border:1px solid var(--line);background:var(--veil-node);text-decoration:none;transition:border-color .2s,background .2s}
.chub-card:hover{background:var(--surface);border-color:var(--amber)}
...
.chub-foot{padding:26px clamp(22px,7vw,110px);border-top:1px solid var(--line);font-family:'Spline Sans Mono',monospace;font-size:9px;letter-spacing:.1em;text-transform:uppercase;color:var(--faint);display:flex;justify-content:space-between;gap:20px}.chub-foot a{color:var(--dim);text-decoration:none}
`;
